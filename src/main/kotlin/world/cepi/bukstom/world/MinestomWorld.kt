...
class MinestomWorld(val server: Server, val instance: Instance, val creator: WorldCreator?) : World {
	...
	override fun isChunkGenerated(x: Int, z: Int): Boolean {
		return instance.getChunk(x, z) != null
	}
	...
	override fun getChunkAtAsync(x: Int, z: Int, gen: Boolean, urgent: Boolean): CompletableFuture<Chunk> {
		...
		val chunk = instance.getChunk(x, z)
		if (chunk != null)
			future.complete(MinestomChunk(this, chunk))

		instance.loadChunk(x, z).thenApplyAsync {
			future.complete(MinestomChunk(this, it))
		}
